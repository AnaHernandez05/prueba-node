...
return true;
   ...
   autosQuePuedeComprar: function(persona){
      let posibles = [];
      let autosDis = this.autosParaLaVenta();
      for(let i=0; i<autosDis.length;i++){
         if((persona.capacidadDePagoEnCuotas*autosDis[i].cuotas>autosDis[i].precio) && (persona.capacidadDePagoTotal>=autosDis[i].precio)){
            posibles.push(autosDis[i]);
      }
   }
   return posibles;
   }
